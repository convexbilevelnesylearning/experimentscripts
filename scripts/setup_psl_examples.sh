...
function fetch_jar() {
    ...
    # psl 3.0.0
    local snapshotJARPath="$HOME/.m2/repository/org/linqs/psl-cli/3.0.0-SNAPSHOT/psl-cli-3.0.0-SNAPSHOT.jar"
    cp "${snapshotJARPath}" psl_resources/psl-cli-3.0.0-SNAPSHOT.jar
}
...
function standard_fixes() {
    ...
    for exampleDir in `find ${PSL_EXAMPLES_DIR} -maxdepth 1 -mindepth 1 -type d -not -name '.*' -not -name '_scripts'`; do
        ...
        pushd . > /dev/null
            ...
            # cp 3.0.0 snapshot into the cli directory.
            cp ../../../psl_resources/psl-cli-3.0.0-SNAPSHOT.jar ./
